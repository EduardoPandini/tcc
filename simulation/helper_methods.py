...
def path_string(path):
    path = str(path)
    return(conta_saltos(path))

def conta_saltos(s: str) -> int:
    # Apagar tudo depois do segundo ']'
    count = 0
    index = 0
    ...
    # Remover todas as ocorrências de '[' e ']'
    s = s.replace('[', '').replace(']', '').replace('(', '')
    # Contar quantos números tem na string
    numbers = s.split(',')
    # print(s)
    count_numbers = sum(1 for num in numbers if num.strip().isdigit())
    # print(count_numbers)
    # print("um deles")
...
def gerar_grafico_contagem(caminhos_contados):
    # Extrair os pares de nós e as contagens
    pares = list(caminhos_contados.keys())
    contagens = list(caminhos_contados.values())

    # # Criar o gráfico de barras
    # plt.figure(figsize=(10, 6))  # Tamanho da figura
    # plt.bar(range(len(pares)), contagens, align='center', color='skyblue')  # Criar as barras
    # plt.xticks(range(len(pares)), pares, rotation='vertical')  # Rotular os eixos X com os pares de nós
    # plt.xlabel('Pares de nós')
    # plt.ylabel('Contagem de ocorrências')
    # plt.title('Contagem de ocorrências de pares de nós modeloteste')
    # plt.tight_layout()  # Ajustar o layout para garantir que tudo seja exibido corretamente
...
def append_to_excel(file_path, new_data):
    ...
    if os.path.exists(file_path):
        ...
        # Se o arquivo não existir, apenas use o novo DataFrame
        updated_df = new_data
    
    # Salvar o DataFrame atualizado de volta no arquivo Excel
    updated_df.to_excel(file_path, index=False)

def contar_pares_ocorrencias(lista_de_caminhos_do_usuario, contador):
    for caminho in lista_de_caminhos_do_usuario:
        # Verificar se caminho é uma lista
        if isinstance(caminho, list):
            # Criar pares de nós consecutivos no caminho
            pares = [(caminho[i], caminho[i+1]) for i in range(len(caminho)-1)]
