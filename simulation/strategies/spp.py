    ...
    try:    
        ...
        for service in services:
            ...
            if not service.being_provisioned:
                app = service.application
                user = app.users[0]
                # Criando um dicionário para armazenar o número de serviços da mesma aplicação em cada servidor
                services_per_server = {server: 0 for server in EdgeServer.all()}
                
                # conta quantos serviços do mesmo tipo estão em cada server e coloca num dict
                for server in services_per_server.keys():
                    ...
                    edge_servers = sorted(services_per_server.keys(), key=lambda server: (
                        calculate_path_delay(origin_network_switch=user.base_station.network_switch, target_network_switch=server.network_switch)))




                # Iterando sobre os servidores ordenados
        ...
        excel_path = os.path.join(logs_dir, 'path_spp.xlsx')
        ...
        excel_path = os.path.join(logs_dir, 'servi_spp.xlsx')
